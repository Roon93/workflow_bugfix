...
class IndexBuilder {
  ...
  indexFile(filePath, repoRoot = '') {
    try {
      ...
      // 跳过大文件（> 1MB），避免 tree-sitter OOM 或超时
      ...
      // 跳过二进制文件：前 8KB 内出现 null byte 即视为二进制
      const probe = raw.slice(0, 8192);
    ...
    while (stack.length > 0) {
      ...
      if (symType) {
        ...
        if (nameNode) {
          ...
          this.db.prepare('INSERT INTO symbols (name, type, file, line) VALUES (?, ?, ?, ?)').run(name, symType, filePath, node.startPosition.row + 1);
        }
      }
