...
// better-sqlite3 is NOT loaded here — worker threads also execute this file,
// and loading the native addon in a worker thread is unsafe.
...
if (!isMainThread) {
  ...
  function extractSymbols(rootNode, content) {
    ...
    while (stack.length > 0) {
      ...
      if (symType) {
        // C/C++ 函数：declarator 是 function_declarator，需要找其中的 identifier
        const nameNode = node.childForFieldName('name') || findIdentifier(node.childForFieldName('declarator'));
  ...
  // 从 declarator 节点向下找第一个 identifier
  ...
  function findEnclosingFunction(node, content) {
    ...
    while (cur) {
      if (cur.type === 'function_definition' || cur.type === 'method_definition') {
        const n = cur.childForFieldName('name') || findIdentifier(cur.childForFieldName('declarator'));
  ...
  parentPort.on('message', ({ batch, repoRoot, done }) => {
    if (done) { parentPort.close(); return; }
...
class IndexBuilder {
  ...
  async indexDirectory(dirPath, repoRoot = '') {
    ...
    // ctags/cscope 轻量索引路径
    if (cfg.indexer === 'ctags' || cfg.indexer === 'ctags+cscope') {
      const { CtagsIndexer } = require('./ctags-indexer.js');
      const indexer = new CtagsIndexer(this.db, this.stmts);
      process.stderr.write(`[index-builder] indexer: ${cfg.indexer}\n`);
      return indexer.indexDirectory(dirPath, root, cfg);
    }
    ...
    const auto = Math.max(1, Math.floor(os.cpus().length * 0.75));
    const numWorkers = cfg.maxWorkers > 0 ? Math.min(cfg.maxWorkers, os.cpus().length) : auto;
    ...
    const writeBatch = this.db.transaction((results) => {
      for (const r of results) {
        ...
        const isCompiled = buildInfo ? (buildInfo.compiled.has(path.resolve(root, r.relPath)) ? 1 : 0) : 1;
    ...
    await Promise.all(workerFiles.map((files) => new Promise((resolve, reject) => {
      ...
      const sendNext = () => {
        if (cursor >= files.length) {
          if (pending === 0) { worker.postMessage({ done: true }); }
          return;
        }
      ...
      worker.on('message', (results) => {
        ...
        if (Math.floor(done / 5000) > Math.floor((done - results.length) / 5000)) {
  ...
  indexFile(filePath, repoRoot = '') {
    ...
    this.db.transaction(() => {
      try {
        ...
        const findId = (node) => { if (!node) return null; if (node.type === 'identifier') return node; for (const c of node.children) { const f = findId(c); if (f) return f; } return null; };
        const stack = [tree.rootNode];
        while (stack.length) {
          const node = stack.pop();
          const st = SYMBOL_TYPES[node.type];
          if (st) { const n = node.childForFieldName('name') || findId(node.childForFieldName('declarator')); if (n) this.stmts.insSymbol.run(content.substring(n.startIndex, n.endIndex), st, relPath, node.startPosition.row + 1); }
